    ...
    for (int i = 0; i<IP_PORTMAP_MAX; i++) {
        if (portmap_tab[i].valid) {
            ...
            if (portmap_tab[i].iface == 1) {
                ...
                } else if (portmap_tab[i].iface == 2) {
                    bind_ip = my_ap_ip;   // AP interface
                } else {
                    bind_ip = my_ip;      // STA interface
                }
...
esp_err_t get_portmap_tab() {
    ...
    for (int i = 0; i < IP_PORTMAP_MAX; i++) {
        if (portmap_tab[i].valid && portmap_tab[i].iface > 2) {
    ...
    for (int i = 0; i<IP_PORTMAP_MAX; i++) {
        if (!portmap_tab[i].valid) {
            ...
            if (bind_ip != 0) {
                ...
                ip4_addr_t dbg_addr;
                dbg_addr.addr = bind_ip;
                ESP_LOGI(TAG, "add_portmap: iface=%u bind_ip=" IPSTR, iface, IP2STR(&dbg_addr));
            return err;
        }
    }
...
esp_err_t del_portmap(u8_t proto, u16_t mport, u8_t iface) {
    for (int i = 0; i < IP_PORTMAP_MAX; i++) {
        if (portmap_tab[i].valid &&
            portmap_tab[i].mport == mport &&
            portmap_tab[i].proto == proto &&
