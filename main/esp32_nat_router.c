...
// static void udp_proxy_task(void *arg) { udp_proxy_start(8899, "192.168.178.20", 8899); vTaskDelete(NULL); }
...
void wifi_init(const uint8_t* mac, const char* ssid, const char* ent_username, const char* ent_identity, const char* passwd, const char* static_ip, const char* subnet_mask, const char* gateway_addr, const uint8_t* ap_mac, const char* ap_ssid, const char* ap_passwd, const char* ap_ip)
{
    ...
    // hier       
    vTaskDelay(pdMS_TO_TICKS(3000));  // wichtig!
...
void app_main(void)
{
    ...
    esp_log_level_set("*", ESP_LOG_VERBOSE); // Log-Level deaktivieren
    ...
    if (boot_feature_enabled(FEATURE_LED_STRIP)) {
    ...
    // pthread_t t1;
    // pthread_create(&t1, NULL, led_status_thread, NULL); //deaktiviert aus Performancegründen (LED-Status)
    ...
    if (boot_feature_enabled(FEATURE_PCAP)) {
        pcap_init();
    }

    // Initialize remote console (TCP server on port 2323, disabled by default)
    if (boot_feature_enabled(FEATURE_REMOTE_CONSOLE)) {
        remote_console_init();
    }

    // Initialize syslog client (UDP forwarding, disabled by default)
    if (boot_feature_enabled(FEATURE_SYSLOG)) {
        syslog_init();
    }

    // Initialize OLED display (ESP32-S3 defaults to enabled on GPIO17/18)
    if (boot_feature_enabled(FEATURE_OLED)) {
        // oled_display_init();
    }

    if (boot_feature_enabled(FEATURE_CONSOLE)) {
        initialize_console();
        /* Register commands */
...
#if CONFIG_MQTT_HOMEASSISTANT
    if (boot_feature_enabled(FEATURE_MQTT_HA)) {
        mqtt_ha_init();
    }
#endif
